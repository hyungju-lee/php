<?php
    ...
    // member 테이블과 css 테이블의 데이터를 가져오므로 163-connection.php 파일을 include 합니다.
        ...
        <div id="contents">
            ...
            <table>
                ...
                <?php
                    ...
                    // 작성자 정보를 함께 표시하지만 css 테이블에는 작성자 정보가 없기 때문에 member 테이블과 함께 memberID 정보를 매칭하여
                    // 작성자 정보를 함께 불러옵니다.
                    $sql = "SELECT b.cssID, b.title, m.nickname, b.regTime FROM css b ";
                    $sql .= "JOIN member m ON (b.memberID = m.memberID) ORDER BY cssID ";
                    ...
                    if ($result) {
                        ...
                        if ($dataCount > 0) {
                            for ($i=0; $i<$dataCount; $i++) {
                                ...
                                echo "<tr>";
                                echo "<td>".$memberInfo['cssID']."</td>";
                                // 제목을 누르면 해당 게시물의 내용을 볼 수 있는 페이지로 이동해야 하므로
                                // a 링크를 사용하여 주소를 지정합니다.
                                // cssID 의 값을 GET 방식으로 전송함을 알 수 있습니다.
                                echo "<td><a href='/wordpress/board/css/view.php?cssID=";
                                echo "{$memberInfo['cssID']}'>";
                                echo $memberInfo['title'];
                                echo "</a></td>";
                                echo "<td>{$memberInfo['nickname']}</td>";
                                // css 테이블에는 게시글을 작성한 시간이 타임스탬프로 되어 있으므로 이 값을 보기 쉽게
            ...
            <?php
                ...
                include '../css/nextPageLink.php';
                include '../css/searchForm.php';
            ?>
        </div>
    </div>
    <?php
