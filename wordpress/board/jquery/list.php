<?php
    ...
    // member 테이블과 jquery 테이블의 데이터를 가져오므로 163-connection.php 파일을 include 합니다.
        ...
        <div id="contents">
            ...
            <table>
                ...
                <?php
                    ...
                    // 작성자 정보를 함께 표시하지만 jquery 테이블에는 작성자 정보가 없기 때문에 member 테이블과 함께 memberID 정보를 매칭하여
                    // 작성자 정보를 함께 불러옵니다.
                    $sql = "SELECT b.jqueryID, b.title, m.nickname, b.regTime FROM jquery b ";
                    $sql .= "JOIN member m ON (b.memberID = m.memberID) ORDER BY jqueryID ";
                    ...
                    if ($result) {
                        ...
                        if ($dataCount > 0) {
                            for ($i=0; $i<$dataCount; $i++) {
                                ...
                                // jqueryID 의 값을 GET 방식으로 전송함을 알 수 있습니다.
                                echo "<td><a href='/wordpress/board/jquery/view.php?jqueryID=";
                                echo "{$memberInfo['jqueryID']}'>";
                                echo $memberInfo['title'];
                                echo "</a></td>";
                                echo "<td>{$memberInfo['nickname']}</td>";
                                // jquery 테이블에는 게시글을 작성한 시간이 타임스탬프로 되어 있으므로 이 값을 보기 쉽게
            ...
            <?php
                ...
                include '../jquery/nextPageLink.php';
                include '../jquery/searchForm.php';
            ?>
        </div>
    </div>
    <?php
