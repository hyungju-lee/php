<?php
    ...
    // 전체 레코드 수 구하기
    // jquery 테이블의 레코드 수를 불러오는 쿼리문입니다.
    $sql = "SELECT count(jqueryID) FROM jquery";
    // 쿼리문을 실행합니다.
    ...
    // 쿼리문의 데이터를 변수 jqueryTotalCount 에 대입합니다.
    $jqueryTotalCount = $result->fetch_array(MYSQLI_ASSOC);
    // 변수 jqueryTotalCount 의 레코드 수 정보를 변수 jqueryTotalCount 에 다시 대입합니다.
    $jqueryTotalCount = $jqueryTotalCount['count(jqueryID)'];
    ...
    $totalPage = ceil($jqueryTotalCount / $numView);
