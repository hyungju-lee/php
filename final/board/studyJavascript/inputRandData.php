<?php
     ...
     * 게시물 입력폼이 있는 페이지로 여러 개의 게시물을 직접 입력하려면 불편하므로 프로그래밍을 통해 임의로 javascript 테이블에
     * 데이터를 입력하겠습니다.
     *
     * 다음은 javascript 테이블에 임의로 데이터를 입력하는 예제입니다.
     * */
    ...
    for ($i=1; $i<165; $i++) {
        ...
        $time = time();
        // javascript 테이블에 입력할 쿼리문입니다.
        // 제목에는 변수 i의 값을 사용하여 몇 번째 제목이라는 문구가 입력되며
        // 내용에는 변수 i의 값을 사용하여 몇 번째 내용이라는 문구가 입력됩니다.
        $sql = "INSERT INTO studyJavascript (memberID, title, content, regTime)";
        $sql .= "VALUES (1, '{$i}번째 제목', '{$i}번째 내용', {$time})";
        $result = $dbConnect->query($sql);

        if ($result) {
            echo "{$i}번째 js데이터 입력완료";
        } else {
            echo "{$i}번째 js데이터 입력실패";
        }
    }
?>
